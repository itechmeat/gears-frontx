...
- The theme an interface opens in is one of the themes it supports, never the set. A pass that captured only that theme has seen a fraction of the surface: switch into each remaining theme and capture it there. A run that reported a verified interface while a whole theme stayed unopened claimed a coverage it never had
...
- The verification report states its coverage as a list, not as a verdict: every theme the interface supports, each marked verified or not-opened, and beside them the states captured. A report without that list is incomplete whatever it concludes. An unopened theme is a legitimate outcome that the list records; a closing summary phrased so that no reader can tell it apart from full coverage is how two consecutive runs shipped a single theme as a verified interface
...
- Before launching a browser, probe once whether a debug port is already listening on 9222 by asking its `/json/version` endpoint, and attach to that browser with `npx --yes agent-browser connect 9222` when it answers. Attaching returned every screenshot in the run that used it, where a self-launched browser hung three times in the run before, and three later runs never made the probe at all
- The browser's text-wait does not see into a shadow root: a wait on rendered text times out while that same text is on screen, twice at 25 seconds across two runs. Wait on the accessibility snapshot instead, which does reach inside
