...
17. [ ] - `p1` - The verification drives every interaction each realized unit declares against the running page, keyboard-operated ones included, and reads back what the page renders rather than what the application's state holds. The two diverge: a control that has stopped answering external state updates that state correctly and repaints nothing, and unit tests cannot detect it, since each test renders the control fresh and that fresh render is the remount which hides the fault. - `inst-sfi-drive-interactions`
18. [ ] - `p1` - **IF** the intent, or a document it referenced, named a design for what was built, the verification additionally compares each realized screen against its design frame: rendered at the frame's own viewport, captured, and the capture examined against the frame as images. Every geometry difference found - element size, placement, alignment, control width, the rendering of a selected state - is corrected before the screen is reported done; a difference the component library cannot express is recorded and closed with that constraint named. A design named and not compared against leaves its screens unverified against it, and is reported so rather than passed: a screen whose design tokens resolve exactly can still render every box at the wrong size, and a walk that checks console output and interaction alone passes it. - `inst-sfi-compare-design`
19. [ ] - `p1` - The entry point creates the verification coverage record, and the capture directory beside it, **before the verification takes its first capture**, and fills the record as each screen and theme is finished. The record is a precondition of the walk rather than a task closing it: a run that reached the walk with the record listed as a closing task drove the walk in full, wrote its captures outside the project, and produced no record at all, while producing the design mapping that the same run's own gate required before it could begin. An obligation at the end of a long run is the obligation that is dropped. The same opening act materializes, inside that record, one open row per category of the verification checklist, transcribed from the checklist itself: the read is made to produce structure because a read that produces nothing is a read that does not happen - a later run wrote the record this gate requires and still never opened the checklist, leaving every category without a verdict and the walk steered by nothing. A row list drawn from memory rather than from the file is self-reporting, since the category ids exist only there. - `inst-sfi-open-coverage`
20. [ ] - `p1` - The entry point completes that verification coverage record before the report is composed - carrying, per screen, what was verified and, where a design was compared against, the comparison outcome with each difference's design value, built value, and fixed-or-closed disposition. The record is owed whichever procedure performed the verification, this feature's own or a covering skill's declared one, because it answers what was verified rather than how; the captures it cites are written inside the project rather than to a temporary location, so a reader can still open them after the session ends. - `inst-sfi-write-coverage`
21. [ ] - `p1` - **IF** a declared verification reports failure - `inst-sfi-if-verify-failed`
...
22. [ ] - `p1` - The entry point reports as the work remaining only the intent that no applied template's ground contains and no activated extension skill covers; it neither modifies any applied template's content as part of applying it nor retries a failed command in a correction loop. - `inst-sfi-report-residual`
23. [ ] - `p1` - **RETURN** scaffolding complete - the applied set as read from provenance, the units realized inside the applied ground and verified, plus the residual work. - `inst-sfi-return-done`
...
- [ ] The coverage record and the capture directory are created before the verification's first capture, so a walk cannot proceed without them, and the record is filled as the walk goes rather than composed after it
- [ ] That same opening act writes one open row per verification-checklist category, transcribed from the checklist, so the walk cannot begin without the checklist having been read; a category still open when the report is composed is reported as not established
- [ ] A region the design lays out as a row of repeated cells was measured as a row - equal cell widths, equal gaps, row edges flush with the content column, end labels anchored to those edges - and not only cell by cell
- [ ] Every interaction a realized unit declares - keyboard-operated ones included - was driven against the running page and its effect confirmed from what the page renders, not from the state the application holds
