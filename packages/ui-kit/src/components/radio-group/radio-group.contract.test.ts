...
function compileUnit(stem: string): CompiledUnit {
  ...
  // Both exports here render a real element (root wraps Base UI's
  // RadioGroup, item wraps Base UI's Radio.Root), so elementKind is never
...
describe('radio-group family in a GTS store', () => {
  function registeredStore(): GTS {
    ...
    // The element surfaces this family's two components name: the root
    // renders a <div>, the item a <span>, so there are two distinct
    // schemas across two components - de-duplicated by $id, because
    // registering the same one twice is not a fact about the family.
