...
function compileUnit(stem: string): CompiledUnit {
  ...
  // Every export here renders a real native element (this component wraps no
  // primitive - it is pure styling translation over native markup), so
  // elementKind is never undefined - a defensive message beats a bare
...
describe('native-select family: what the schema cannot assert', () => {
  ...
  it('files the root redeclared props as properties and the native attributes as forwarded surface', () => {
    ...
    expect(rootProps.size).toEqual({ type: 'string', enum: ['sm', 'default'] });
...
describe('native-select family in a GTS store', () => {
  function registeredStore(): GTS {
    ...
    // The element surfaces this family's three components name: the root
    // renders a <select>, the option an <option> and the group an
    // <optgroup>, so there are three distinct schemas across three
    // components - de-duplicated by $id, because registering the same one
    // twice is not a fact about the family.
