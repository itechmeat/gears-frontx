...
function compileUnit(stem: string): CompiledUnit {
  ...
  // The root wraps Base UI's ToggleGroup (a div), the item Base UI's Toggle
  // (a button), so elementKind is never undefined - a defensive message
...
describe('toggle-group family in a GTS store', () => {
  function registeredStore(): GTS {
    ...
    // The root renders a <div>, the item a <button>: two distinct surfaces,
    // de-duplicated by $id all the same.
