...
module.exports = {
  ...
  options: {
    ...
    // `../node_modules` in a tree that sits inside a larger repo. It also misses
    // the trees `frontx add mfe` (and CI's composition of template-mfe onto this
    // shell) creates, where `npm install` nests a `node_modules` under every
    // `src-app/mfe_packages/*` workspace whose pins conflict with the shell's.
    // Measured on this template with the anchored pattern, 1268 of 1670 modules
    // came from nested trees and 958 more from `../node_modules` — third-party
    // code no rule below can fire on, type-parsed anyway because
    // `tsPreCompilationDeps` runs the TS compiler over every module. In a
    // scaffolded project that traversal is not a rule violation but an OOM.
    //
